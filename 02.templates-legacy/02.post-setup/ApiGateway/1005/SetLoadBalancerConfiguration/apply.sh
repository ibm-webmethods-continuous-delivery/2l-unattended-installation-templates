...
if [ ! -f "${WMUI_APIGW_LB_JSON_FILE}" ]; then
    logE "The expected json configuration file for Load Balancers was not found: ${WMUI_APIGW_LB_JSON_FILE}"
    exit 4
fi

URL="${WMUI_APIGW_URL_PROTOCOL}://${WMUI_APIGW_DOMAINNAME}:${WMUI_APIGW_SERVICE_PORT}/rest/apigateway/configurations/loadBalancer"
...
curl -u "Administrator:${WMUI_APIGW_ADMINISTRATOR_PASSWORD}" \
    ...
    --silent -o "${WMUI_AUDIT_SESSION_DIR}/LB_BEFORE_CHANGE_AT_${TMP_NOW}.json"

if [ ! -f "${WMUI_AUDIT_SESSION_DIR}/LB_BEFORE_CHANGE_AT_${TMP_NOW}.json" ]; then
    logE "Given Administrator password is not currently valid. Cannot continue"
    exit 3
fi

envsubst \
    < "${WMUI_APIGW_LB_JSON_FILE}" \
    > "${WMUI_AUDIT_SESSION_DIR}/LB_PUT_AT_${TMP_NOW}.json"
...
curlCmd=${curlCmd}' -d "@/${WMUI_AUDIT_SESSION_DIR}/LB_PUT_AT_${TMP_NOW}.json"'
...
curl -u "Administrator:${WMUI_APIGW_ADMINISTRATOR_PASSWORD}" \
    "${URL}" \
    -H "Accept: application/json" \
    --silent -o "${WMUI_AUDIT_SESSION_DIR}/LB_AFTER_CHANGE_AT_${TMP_NOW}.json"
