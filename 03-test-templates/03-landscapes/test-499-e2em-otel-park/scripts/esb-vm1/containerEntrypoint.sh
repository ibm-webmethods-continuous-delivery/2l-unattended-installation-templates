...
onInterrupt(){
	pu_log_i "[$lLOG_PREFIX:onInterrupt()] - Interrupted! Shutting down MSR"
...
setupMSR() {
  pu_log_i "[${lLOG_PREFIX}:setupMSR()] - Checking if MSR is already installed..."
  
  # Check if MSR is already installed__err_count
  if [ ! -f "${WMUI_INSTALL_INSTALL_DIR}/IntegrationServer/bin/startup.sh" ]; then
    pu_log_i "[${lLOG_PREFIX}:setupMSR()] - MSR is not present, setting up..."
    
    # Wait for database to be available
    pu_log_i "[${lLOG_PREFIX}:setupMSR()] - Waiting for database to be available -> ${WMUI_LAB01_DBSERVER_HOSTNAME}:${WMUI_LAB01_DBSERVER_PORT}..."
    while ! portIsReachable2 "${WMUI_LAB01_DBSERVER_HOSTNAME}" "${WMUI_LAB01_DBSERVER_PORT}"; do
        pu_log_i "[${lLOG_PREFIX}:setupMSR()] - Waiting for the database to come up, sleeping 5..."
        sleep 5
    ...
    pu_log_i "[${lLOG_PREFIX}:setupMSR()] - MSR installation found, skipping setup."
  fi
}

startMSR() {
  refreshPubSubMonPackages
  pu_log_i "[${lLOG_PREFIX}:startMSR()] - Starting MSR..."
  ...
  while [ $retries -lt $max_retries ]; do
    if curl -u "Administrator:manage" --write-out 'HTTP %{http_code}' --fail --silent --output /dev/null http://localhost:5555/invoke/wm.server/ping 2>/dev/null; then
      pu_log_i "[${lLOG_PREFIX}:startMSR()] - MSR is ready!"
      break
    fi
    pu_log_i "[${lLOG_PREFIX}:startMSR()] - Waiting for MSR to start... (attempt $((retries + 1))/$max_retries)"
    sleep 10
    retries=$((retries + 1))
  done
  
  if [ $retries -eq $max_retries ]; then
    pu_log_e "[${lLOG_PREFIX}:startMSR()] - MSR failed to start within expected time!"
    return 32
  fi
}
...
showAccessInfo() {
  ...
  pu_log_i "[${lLOG_PREFIX}:showAccessInfo()] - Lab 10 - E2E Monitoring with Instana"
  pu_log_i "[${lLOG_PREFIX}:showAccessInfo()] - ==================================================="
  pu_log_i "[${lLOG_PREFIX}:showAccessInfo()] - MSR Admin UI: http://${WMUI_LAB01_HOST_NAME}:${WMUI_LAB01_PORT_PREFIX}55"
...
refreshPubSubMonPackages(){
  if [ "${WMUI_LAB01_ESB_COPY_PUB_SUB_MON_PACKAGES}" = "true" ]; then
    pu_log_i "[${lLOG_PREFIX}:refreshPubSubMonPackages()] - Refreshing packages from the publish subscribe with monitoring service development template..."
    local __src_packages_home="${WMUI_LAB01_PUB_SUB_MON_01_REPO_MOUNT_POINT}/01.code/is-packages"
    if [ ! -d "${__src_packages_home}" ]; then
      pu_log_e "[${lLOG_PREFIX}:refreshPubSubMonPackages()] - Folder does not exist: ${__src_packages_home}!"
      pu_log_e "[${lLOG_PREFIX}:refreshPubSubMonPackages()] - Is the repository 5s-pub-sub-with-mon-01 correctly mounted?"
      return 1
    fi
...
# Main execution flow
main() {
  pu_log_i "[${lLOG_PREFIX}:main()] - Starting MSR selection 20250924 setup..."
  ...
  pu_log_i "[${lLOG_PREFIX}:main()] - Waiting on MSR process (PID: ${MSR_PID})..."
  wait "${MSR_PID}"
}
